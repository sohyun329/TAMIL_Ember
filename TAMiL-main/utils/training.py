    ...
    for t in range(dataset.N_TASKS):
        ...
        if t:
            ...
            for ema_model in lst_ema_models:
                if hasattr(model, ema_model):
                    ...
                    if len(ema_results[ema_model]) < t:
                        ema_results[ema_model].append(ema_accs[0])
                    else:
                        ema_results[ema_model][t - 1] += ema_accs[0]
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in enumerate(train_loader):
                # if train_loader is not None and hasattr(train_loader.dataset, 'logits'):
